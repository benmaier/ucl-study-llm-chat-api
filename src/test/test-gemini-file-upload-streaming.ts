/**
 * Test: Google Gemini File Upload with Streaming
 *
...
function handleStreamEvent(event: StreamEvent) {
  switch (event.type) {
    ...
    case "code":
      ...
      break;
    case "code_complete":
      console.log(`\n${colors.magenta}✓ [Code execution finished]${colors.reset}`);
      break;
    case "code_output":
      if (event.output) {
        const preview = event.output.slice(0, 200) + (event.output.length > 200 ? "..." : "");
...
async function main() {
  ...
  console.log(`${colors.bright}Gemini File Upload with Live Streaming${colors.reset}`);
  console.log(`${colors.bright}${"=".repeat(70)}${colors.reset}`);
  console.log(`
${colors.dim}Legend:${colors.reset}
  ...
  ${colors.cyan}Cyan${colors.reset}     = Code being executed
  ${colors.dim}Dim${colors.reset}      = Execution output
`);
