/**
 * Test: OpenAI File Upload with Streaming
 *
 * Demonstrates live streaming of code execution with file upload.
 * Shows real-time text, code generation, and execution status.
 */
...
function handleStreamEvent(event: StreamEvent) {
  switch (event.type) {
    ...
    case "code":
      // Code being written - show in cyan
      process.stdout.write(`${colors.cyan}${event.code}${colors.reset}`);
      break;

    case "code_executing":
      console.log(`\n${colors.magenta}⚙ [Executing code...]${colors.reset}`);
      break;
    ...
    case "tool_end":
      ...
      console.log(`${colors.yellow}${colors.bright}✓ [${event.toolName} completed]${colors.reset}\n`);
      break;
  }
}
...
async function main() {
  ...
  console.log(`${colors.bright}OpenAI File Upload with Live Streaming${colors.reset}`);
  console.log(`${colors.bright}${"=".repeat(70)}${colors.reset}`);
  console.log(`
${colors.dim}Legend:${colors.reset}
  ...
  ${colors.cyan}Cyan${colors.reset}    = Code being written
  ${colors.magenta}Magenta${colors.reset} = Code execution status
`);
...
Show your work step by step.`,
    ...
    if (result.codeArtifacts.length > 0) {
      ...
      for (const artifact of result.codeArtifacts) {
        ...
        console.log(`   ${colors.dim}${artifact.path} (${lines} lines)${colors.reset}`);
      }
    }
